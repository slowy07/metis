...
- **Conan support** — `metis init --enable-conan` generates a `conanfile.py` alongside CMake scaffolding.
- **Own pre-commit now runs clang-tidy** — `.metis.toml` gained a `clang-tidy` check; `tests/.clang-tidy` keeps test sources on a more permissive preset.
