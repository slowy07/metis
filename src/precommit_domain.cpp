...
# Run each check in parallel, collect exit codes
declare -a PIDS=()
declare -a CHECK_NAMES=()

for check in $(sniffercommit run --dry-run --staged 2>/dev/null | grep -E '^\s+\[' || true); do
    # Background execution of individual checks
    : 
done

# KISS: Delegate actual parallelization to sniffercommit binary with --parallel flag
exec sniffercommit run --staged --parallel
)",
