...
std::string generate_cmake_lists(const CMakeConfig& cfg) {
  ...
  if (!cfg.description.empty()) {
    ...
    if (!cfg.homepage_url.empty()) {
      cmake += fmt::format("  HOMPAGE_URL \"{}\" \n", cfg.homepage_url);
    }

    cmake += fmt::format("   LANGUAGE CXX\n");
    cmake += fmt::format(")\n\n");
  } else {
  ...
  cmake += fmt::format("set(CMAKE_CXX_STANDARD REQUIRED {})\n",
                       cfg.cpp_standard_required ? "ON" : "OFF");
  cmake += fmt::format("set(CMAKE_CXX_EXTENSION {})\n", cfg.enable_extension ? "ON" : "OFF");
  cmake += "\n";

  if (cfg.build_preset != BuildTypePreset::ReleaseOnly) {
    cmake += fmt::format(
        "# Build type Configuration\n"
        "set(CMAKE_CONFIGURATION TYPES \"{}\" CACHE STRING \"Available build types\" FORCE)\n"
  ...
  if (!cfg.depedencies.empty()) {
    ...
    switch (cfg.dep_strategy) {
      case DepedencyStrategy::FetchContent:
        ...
        for (size_t i = 0; i < cfg.depedencies.size(); ++i) {
          ...
          cmake += cfg.depedencies.at(i);
        }
        cmake += "}\n\n";
        break;

      case DepedencyStrategy::FindPackage:
        for (const auto& dep : cfg.depedencies) {
  ...
  if (cfg.source_files.empty()) {
    cmake += "file(GLOB_RECURSE SOURCES CONFIGURE_DEPENDS \"\n";
    cmake += "   src/*.cpp\n";
    cmake += "   src/*.cc\n";
    cmake += ")\n";
  } else {
  ...
  if (cfg.enable_warnings && cfg.target_type != TargetType::HeaderOnly) {
    ...
    cmake += fmt::format("if(CMAKE_CXX_COMPILER_ID MATCHES \"GNU|Clang\")\n");
  ...
  if (cfg.enable_sanitizers && cfg.target_type != TargetType::HeaderOnly) {
    ...
    cmake += fmt::format(
        ...
        "\"GNU|Clang\")\n");
    cmake += fmt::format("  target_compile_options({} PRIVATE -fsanitize=address,undefined)\n",
                         target_name);
    cmake += fmt::format("  target_link_options({} PRIVATE -fsanitize=address,undefined)\n",
                         target_name);
    cmake += "endif()\n\n";
  }
