...
std::string Depedency::validate() const noexcept {
  if (name.empty()) {
    return "Depedency name cannot be empty";
  }

  if (git_url.empty()) {
    return fmt::format("Depedency `{}` missing git url", name);
  }

  if (!is_valid_url()) {
    return fmt::format("Depedency `{}` has invalid git url: {}", name, git_url);
  }

  if (git_tag.empty()) {
    return fmt::format("Depedency `{}` missing git tag", name);
  }

  return "";
}

bool Depedency::is_valid_url() const noexcept { return is_valid_git_url(git_url); }

bool is_valid_git_url(std::string_view url) noexcept {
  ...
  // NOTE: SSH scheme
  constexpr std::string_view scheme_git = "git@";

  bool has_https = url.size() > scheme_https.size();
  bool has_ssh = url.size() > scheme_git.size();

  if (!has_https && !has_ssh) {
    return false;
  }
  ...
  auto host_end = url.find('/', has_https ? scheme_https.size() : scheme_git.size());

  if (host_end == std::string_view::npos) {
    return false;
  }

  auto repo_start = host_end + 1;
  if (repo_start >= url.size()) {
    return false;
  }
...
std::string CMakeConfig::validate() const noexcept {
  ...
  for (const auto& dep : depedencies) {
...
namespace {
std::string generate_cmake_header(const CMakeConfig& cfg) {
  return fmt::format(
      "# Generated by sniffercommit v0.2.1 | DO NOT EDIT MANUALLY\n"
      "# Project: {} v{}\n"
      "# C++ Standard: {}\n"
...
std::string generate_cmake_project_decl(const CMakeConfig& cfg) {
  std::string cmake;
...
std::string generate_cmake_standard(const CMakeConfig& cfg) {
  std::string cmake;
...
std::string generate_cmake_build_preset(const CMakeConfig& cfg) {
...
std::string generate_cmake_ipo(const CMakeConfig& cfg) {
  if (!cfg.enable_ipo) {
    return "";
  }

  return "include(CheckIPOSupported)\n"
...
std::string generate_cmake_depedencies(const CMakeConfig& cfg) {
  if (cfg.depedencies.empty()) {
    return "";
  }

  std::string cmake = "# Dependencies\n";
  ...
  switch (cfg.dep_strategy) {
    case sniffercommit::tooling::DepedencyStrategy::FetchContent:
      ...
      for (const auto& dep : cfg.depedencies) {
        cmake += fmt::format(
            ...
            "  GIT_SHALLOW ON\n"
            ")\n",
            dep.name, dep.git_url, dep.git_tag);
      }
    ...
    case sniffercommit::tooling::DepedencyStrategy::Conan:
      ...
      for (const auto& dep : cfg.depedencies) {
        cmake += fmt::format("find_package({} CONFIG REQUIRED)\n", dep.name);
      }
      cmake += "\n";
      break;
    case sniffercommit::tooling::DepedencyStrategy::Vcpkg:
      cmake += "# vcpkg integration\n";
      cmake += "# make sure VCPKG_ROOT is set in environment\n";
      for (const auto& dep : cfg.depedencies) {
        cmake += fmt::format("find_package({} CONFIG REQUIRED)\n", dep.name);
      }
...
std::string generate_cmake_sources(const CMakeConfig& cfg) {
  std::string cmake = "# Source files\n";
...
std::string generate_cmake_target(const CMakeConfig& cfg) {
  std::string target_name = cfg.target_name.empty() ? cfg.project_name : cfg.target_name;
  std::string cmake = "# Target definition\n";
...
std::string generate_cmake_link_deps(const CMakeConfig& cfg) {
  if (cfg.depedencies.empty() || cfg.target_type == TargetType::HeaderOnly) {
    return "";
  }

  std::string target_name = cfg.target_name.empty() ? cfg.project_name : cfg.target_name;
  std::string cmake = fmt::format("target_link_libraries({} PUBLIC\n", target_name);
...
std::string generate_cmake_warnings(const CMakeConfig& cfg) {
  if (!cfg.enable_warnings || cfg.target_type == TargetType::HeaderOnly) {
    return "";
  }

  std::string target_name = cfg.target_name.empty() ? cfg.project_name : cfg.target_name;
  std::string cmake = "# Compiler warnings\n";
...
std::string generate_cmake_sanitizers(const CMakeConfig& cfg) {
...
std::string generate_cmake_clang_tidy(const CMakeConfig& cfg) {
  if (!cfg.enable_clang_tidy || cfg.target_type == TargetType::HeaderOnly) {
    return "";
  }

  std::string target_name = cfg.target_name.empty() ? cfg.project_name : cfg.target_name;
  return "# clang-tidy integration\n"
         "find_program(CLANG_TIDY_EXE NAMES clang-tidy)\n"
         "if(CLANG_TIDY_EXE)\n" +
         fmt::format("  set_target_propeties({} PROPERTIES\n", target_name) +
         "   CXX_CLANG_TIDY "
...
std::string generate_cmake_clang_format(const CMakeConfig& cfg) {
  if (!cfg.enable_clang_format) {
    return "";
  }
  return "# clang-format integration\n"
         "find_program(CLANG_FORMAT_EXE NAMES clang-format)\n"
         "if(CLANG_FORMAT_EXE)\n"
         "  file(GLOB_RECURSE ALL_SOURCES src/*.cpp src/*.hpp include/*.hpp)\n"
...
std::string generate_cmake_testing(const CMakeConfig& cfg) {
...
std::string generate_cmake_lists(const CMakeConfig& cfg) {
  ...
  std::string cmake;
  cmake += generate_cmake_header(cfg);
  cmake += "cmake_minimum_required(VERSION 3.20)\n";
  cmake += generate_cmake_project_decl(cfg);
  cmake += generate_cmake_standard(cfg);
  cmake += generate_cmake_build_preset(cfg);

  if (cfg.export_compile_commands) {
    cmake += "set(CMAKE_EXPORT_COMPILE_COMMANDS ON)\n\n";
  }

  cmake += generate_cmake_ipo(cfg);
  cmake += generate_cmake_depedencies(cfg);
  cmake += generate_cmake_sources(cfg);
  cmake += generate_cmake_target(cfg);
  cmake += generate_cmake_link_deps(cfg);
  cmake += generate_cmake_warnings(cfg);
  cmake += generate_cmake_sanitizers(cfg);
  cmake += generate_cmake_clang_tidy(cfg);
  cmake += generate_cmake_clang_format(cfg);
  cmake += generate_cmake_testing(cfg);
  cmake += generate_cmake_install(cfg);
