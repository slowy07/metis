...
if [ "${FORCE_BUILD}" = "true" ]; then
    ...
    mkdir -p "${INSTALL_DIR}"
    if [ -f "${BUILD_DIR}/src/build/bin/sniffercommit" ]; then
        cp "${BUILD_DIR}/src/build/bin/sniffercommit" "${INSTALL_DIR}/sniffercommit"
    elif [ -f "${BUILD_DIR}/src/build/sniffercommit" ]; then
        cp "${BUILD_DIR}/src/build/sniffercommit" "${INSTALL_DIR}/sniffercommit"
    else
        echo "Error: sniffercommit binary not found after build" >&2
        find "${BUILD_DIR}/src/build" -name "sniffercommit*" -type f >&2 || true
        exit 1
    fi
